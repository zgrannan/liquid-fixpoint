    ...
    go ictx =  do let cands = icCands ictx
                  ...
                  if S.null (us `S.difference` icEquals ictx)
                        then return ictx
                                 ...
                                 let newcands = mconcat (makeCandidates γ ictx' <$> S.toList (cands <> ( ((,True) . snd) `S.map` us)))
...
data ICtx    = ICtx
  ...
  , icCands    :: S.HashSet (Expr, Bool)    -- ^ "Candidates" for unfolding
...
updCtx InstEnv {..} ctx delta cidMb
  ...
  where
    initEqs   = S.fromList $ concat [rewrite e rw | e  <- map fst cands ++ (snd <$> S.toList (icEquals ctx))
                                                  , rw <- knSims ieKnowl]
...
makeCandidates :: Knowledge -> ICtx -> Expr -> [(Expr, Bool)]
makeCandidates γ ctx expr
  = mytracepp ("\n" ++ show (length cands) ++ " New Candidates") cands
  where
    cands = map (, True) pleCands ++ map (, False) (rwCands L.\\ pleCands)
    rwCands = filter (\e -> (isRedex γ e || isEq e) && (not (e `S.member` icSolved ctx))) (notGuardedApps expr)
    pleCands = filter (\e -> isRedex γ e && (not (e `S.member` icSolved ctx))) (notGuardedApps expr)
    isEq (EEq _ _) = True
    isEq _         = False
...
evalOne :: Knowledge -> EvalEnv -> ICtx -> (Expr, Bool) -> IO EvAccum
evalOne γ env ctx (e, _) | null $ getAutoRws γ ctx = do
    (e',st) <- runStateT (fastEval γ ctx e) env
    return $ if e' == e then evAccum st else S.insert (e, e') (evAccum st)
evalOne γ env ctx (e,ple) = do
  evAccum <$> execStateT (eval γ ctx ple [(e, PLE)]) env
...
eval :: Knowledge -> ICtx -> Bool -> [(Expr, TermOrigin)] -> EvalST ()
eval _ _ ctx path
...
eval γ ctx usePLE path =
  do
    rws <- getRWs
    e' <-
      if usePLE
        then Just <$> simplify γ ctx <$> evalStep γ ctx e
        else return Nothing
    let evalIsNewExpr = L.notElem e' pathExprs
    return ((if evalIsNewExpr then [e'] else []) ++ map fst rws, evalIsNewExpr)
    else return $ (map fst rws, True)
